...
func _月度派系更新() -> void:
	...
	for 派系 in 派系列表:
		...
		if 满意度 < 30 and randf() < 0.1:
			Game._加推演条目("⚠️ %s人心尽失（%.0f%%），恐生肘腋之变！" % [str(派系.get("名称", "")), 满意度], "宗门警讯", "高")
...
func 制定宗门政策(政策名: String, 持续日: int = 365) -> Dictionary:
	...
	Game._加推演条目("📜 宗主颁令：%s（为期一载，可随时废黜）" % [政策名, 持续日], "宗主颁令", "中")
...
func 提升心腹等级(弟子ID: String) -> Dictionary:
	...
	return {"成功": true, "消息": "%s心腹等级提升为「%s」" % [d.姓名, 等级名]}
...
func 挑起派系对抗(派系ID1: String, 派系ID2: String) -> Dictionary:
	...
	Game._加推演条目("⚔️ 宗主暗中挑动两派相斗，鹬蚌相争" , "挑动相斗", "高")
...
func _月度炼丹派系争夺() -> void:
	...
	Game._加推演条目("⚗️ %s与%s争夺上古丹方，%s技高一筹！" % [str(派系1.get("名称","")), str(派系2.get("名称","")), str(胜者.get("名称",""))], "丹方之争", "中")
