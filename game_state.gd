...
func 挑战关卡(stage_id: String, 出战弟子: Array = [], mode: String = "full") -> Dictionary:
	...
	if 结果["is_win"]:
		...
		for drop in drops:
			...
			if 得主 != null:
				得主.获得物品(it)
				结果["奖励摘要"] += " 得【%s×%d】" % [drop.get("item_name", ""), drop.get("count", 1)]
...
func _资源建筑产出():
	var 经营基数: Dictionary = {"lingtian": 5, "kuangmai": 4, "dantang": 3, "qitang": 3}
...
func 任命负责人(key: String, d: Disciple):
	if not 堂口列表.has(key):
		return
	_设负责人(key, d)
	_加推演条目("【人事】%s 任%s负责人。" % [d.姓名, 堂口列表[key]["名称"]], ET_APPOINT, PRIO_NORMAL, {"弟子": d.姓名, "堂口": key})
	弟子变动.emit()
...
func 举办测灵根(强制天品: bool = false) -> Dictionary:
	...
	for d in 新徒:
		if d.灵根品阶 == "天品":
			宗门纪事.append({"日": 累计游戏日, "弟子": d.姓名, "稀有度": "天品", "名称": "天品灵根弟子", "文案": "测灵大典检出天品灵根弟子【%s】，全宗士气大振。" % d.姓名})
