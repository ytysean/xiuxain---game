...
func 供奉香火(数量: int) -> Dictionary:
	...
	声望 = 声望 + int(数量 / 20)
	添加纪事("庶务", "供奉香火", "供奉香火%d，繁荣+%d、声望+%d" % [数量, int(数量 / 5), int(数量 / 20)], 1)
	return {"成功": true, "消耗香火": 数量, "繁荣": 繁荣, "声望": 声望, "消息": "供奉香火%d，宗门繁荣与声望提升" % 数量}
		...
		if 首通:
			...
			for drop in drops:
				...
				if 得主 != null:
					var 额外件数: int = 0
					if 御兽相助:
						额外件数 = max(1, int(round(drop.get("count") if "count" in drop else 1 * 0.10)))
						for _k in 额外件数:
...
func _资源殿阁产出():
	...
	for key in 司职列表.keys():
		...
		if has_method("取宗主皮肤全宗加成"):
			...
			产出值 = int(n * base * (1.0 + 经营加成) * 气运乘 * (1.0 + 产出buff + 彩蛋产出加成() + 产出池加成 * 等级乘区 * 宗主灵石系数 * _殿阁等级_乘区(key)))
			灵石 += 产出值
