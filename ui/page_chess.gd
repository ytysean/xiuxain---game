...
func _build() -> void:
	...
	add_child(main)
	var 顶栏: PanelContainer = UITheme.建顶栏("论道棋弈", _on返回)
	main.add_child(顶栏)
...
func _建_总览() -> void:
	...
	UITheme.apply_project_font(韵, UITheme.FONT_BODY)
	韵.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_content.add_child(韵)
	韵.modulate.a = 0.0
	韵.create_tween().tween_property(韵, "modulate:a", 1.0, 0.25)
	...
	UITheme.apply_project_font(增标, UITheme.FONT_BODY)
	增标.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_content.add_child(增标)
	增标.modulate.a = 0.0
	增标.create_tween().tween_property(增标, "modulate:a", 1.0, 0.25)
	# P3 论道大会周赛段
	var _fb3 := _分隔("论道大会周赛")
...
func _建_论道() -> void:
	...
	if Game.论道系统.棋类表.is_empty():
		...
		UITheme.apply_project_font(空, UITheme.FONT_AUX)
		_content.add_child(空)
		空.modulate.a = 0.0
		空.create_tween().tween_property(空, "modulate:a", 1.0, 0.25)
	...
	UITheme.apply_project_font(棋, UITheme.FONT_BODY)
	_content.add_child(棋)
	棋.modulate.a = 0.0
	棋.create_tween().tween_property(棋, "modulate:a", 1.0, 0.25)
	...
	求棋.modulate.a = 0.0
	求棋.create_tween().tween_property(求棋, "modulate:a", 1.0, 0.25)
	...
	切棋.modulate.a = 0.0
	切棋.create_tween().tween_property(切棋, "modulate:a", 1.0, 0.25)
	...
	UITheme.apply_project_font(茶, UITheme.FONT_BODY)
	_content.add_child(茶)
	茶.modulate.a = 0.0
	茶.create_tween().tween_property(茶, "modulate:a", 1.0, 0.25)
	...
	求茶.modulate.a = 0.0
	求茶.create_tween().tween_property(求茶, "modulate:a", 1.0, 0.25)
	...
	切茶.modulate.a = 0.0
	切茶.create_tween().tween_property(切茶, "modulate:a", 1.0, 0.25)
