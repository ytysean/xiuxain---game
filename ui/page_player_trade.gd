...
func _build发起交易() -> void:
	...
	运输标签.text = "运输方式："
	运输标签.add_theme_font_size_override("font_size", UITheme.FONT_H2)
	...
	确认提示.text = "⚠️ 发起交易后将扣除运输费用，取消交易仅退还50%。请仔细核对买家和物品。"
	确认提示.add_theme_font_size_override("font_size", UITheme.FONT_BODY)
...
func _build交易卡片(交易: Dictionary, 显示收货按钮: bool) -> void:
	...
	状态标签.add_theme_font_size_override("font_size", UITheme.FONT_H2)
	...
	信息标签.add_theme_font_size_override("font_size", UITheme.FONT_BODY)
	...
	if 状态 == "运输中" or 状态 == "待确认":
		...
		送达标签.add_theme_font_size_override("font_size", UITheme.FONT_BODY)
	...
	if 状态 == "已被劫":
		...
		赔偿标签.add_theme_font_size_override("font_size", UITheme.FONT_BODY)
	...
	if bool(交易.get("货到付款", false)):
		...
		货到标签.text = "（货到付款）"
		货到标签.add_theme_font_size_override("font_size", UITheme.FONT_BODY)
