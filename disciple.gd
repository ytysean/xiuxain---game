...
func _修复装备一致性():
	for 槽 in 装备.keys():
		...
		if it == null:
			装备.erase(槽)
			continue
		# 防御旧档/历史 bug：材料或丹药误穿到装备槽，卸载回背包
		if it.类别 in ["ling_cai", "dan_yao"] or not it.可穿戴():
			背包.append(it)
			装备.erase(槽)
			continue
		if 背包.has(it):
...
func 累计瓶颈打磨(日: float):
	if 瓶颈打磨值 >= 1.0:
		return
	var 自然增量: float = 日 * 资质突破系数.get(资质, 1.0) / 2880.0
...
func 突破():
	...
	稳固期剩余 = 稳固期天数.get(境界, 0.0)  # P1 突破后强制稳固期（效率减半）
	寿元 = 境界表[境界]["寿元"]
	战力 = 境界表[境界]["战力"]
	算属性()
	_刷新身份()
