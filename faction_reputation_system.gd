class_name FactionReputationSystem extends RefCounted
...
func 增加阵营声望(阵营: String, 数量: int) -> void:
	...
	Game.添加纪事("庶务", "阵营声望变化", "%s声望+%d" % [实际阵营, 数量], 1)
# 获取阵营声望权益
...
func 检查阵营任务解锁(阵营: String, 所需声望等级: String) -> bool:
	...
	var 当前索引: int = FactionSystem.REPUTATION_LEVELS.find(当前等级)
	var 所需索引: int = FactionSystem.REPUTATION_LEVELS.find(所需声望等级)
	return 当前索引 >= 所需索引
...
func 领取阵营任务奖励(任务ID: String) -> Dictionary:
	...
	Game.添加纪事("庶务", "阵营任务奖励", "了却%s差事，获得%d灵石、%d灵气、%d声望" % [任务配置.get("quest_name", ""), 灵石奖励, 灵气奖励, 声望奖励], 1)
	return {"成功": true, "Game.灵石": 灵石奖励, "Game.灵气": 灵气奖励, "声望": 声望奖励}
...
func 购买阵营商店商品(商品ID: String) -> Dictionary:
	...
	if not 检查阵营任务解锁(阵营, 所需声望):
		return {"成功": false, "原因": "声望品级不足，商品未解锁"}
	# 计算价格
	...
	if not 阵营商店购买记录.has(商品ID):
		阵营商店购买记录[商品ID] = 0
	阵营商店购买记录[商品ID] += 1
	Game.添加纪事("庶务", "阵营商店购买", "花费%d灵石购买%s" % [实际价格, str(商品配置.get("item_name", 商品ID))], 1)
	return {"成功": true, "花费灵石": 实际价格, "商品": 商品ID}
...
func to_dict() -> Dictionary:
	...
	data["阵营任务进度"] = 阵营任务进度
	data["阵营商店配置"] = 阵营商店配置
	data["阵营商店购买记录"] = 阵营商店购买记录
	return data

func from_dict(data: Dictionary) -> void:
	if "阵营声望" in data: 阵营声望 = data["阵营声望"]
	if "阵营任务配置" in data: 阵营任务配置 = data["阵营任务配置"]
	if "阵营任务进度" in data: 阵营任务进度 = data["阵营任务进度"]
	if "阵营商店配置" in data: 阵营商店配置 = data["阵营商店配置"]
	if "阵营商店购买记录" in data: 阵营商店购买记录 = data["阵营商店购买记录"]
