...
const 作物描述: Dictionary = {
	...
	"灵参": "千年灵参，根须如龙，蕴含磅礴生机。服之可增加寿元，是突破境界、延续生命的稀世珍宝。",
	"灵芝": "赤心灵芝，菌盖如伞，疗伤圣品。可治疗道伤，加速弟子伤势恢复，是宗门医堂的必备药材。",
	"灵桃": "蟠桃灵果，三千年一开花，三千年一结果。食之可增加突破成功率，是冲击境界的无上助力。",
...
func refresh() -> void:
	...
	if _当前标签 == "地块":
		...
		_作物列表 = Game.药园作物配置.keys()
		for i in _作物列表.size():
			var 作物名称 = _作物列表[i]
			var 作物配置 = Game.药园作物配置.get(作物名称, {})
			var 行 = Button.new()
