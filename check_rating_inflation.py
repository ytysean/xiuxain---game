...
# 全程零新增资源；7 年总奖励偏差 > ±5% 即阻断。
#
# 三层校验：
#   A) 配置层：config/评级节奏.csv 的 年度奖励占比 + 七载奖励占比 必须 ≈ 1.0
#      （这是通胀/通缩的根因锁——任一项漂移都会静默改变总产出）
#   B) 公式镜像层：用代表性 7 年评级序列复算「年度 70% 即时 + 池 30% 递延」，
#      断言 7 年总奖励偏差 ≤ 5%（且 年度发+池 == 原总额）
#   C) GDScript 守恒层：静态校验 game_state.gd 的分发逻辑——
#      · 比例来自配置 _校准浮("年度奖励占比") 而非硬编码字面量
#      · 入池 = 奖 - 年度发（精确守恒，无额外灌水）
#      · _七载大考 消费 七载奖励池（灵石 += 大典灵石 后 七载奖励池 = 0）
#
...
def main():
    ...
    try:
        annual = float(kv.get("年度奖励占比", "0.70"))
        sept = float(kv.get("七载奖励占比", "0.30"))
    except ValueError:
        fail("年度奖励占比/七载奖励占比 非数值")
    ...
    # ---------- B) 公式镜像层：7 年总奖励偏差 ≤ 5% ----------
    # 此表镜像 game_state.gd::_发放周期奖励 的 灵石奖励表（改动须同步）
    灵石奖励表 = {"D": 0, "C": 200, "B": 500, "A": 1000,
               "A+": 1800, "S": 3000, "SS": 5000, "SSS": 8000}
    序列 = ["A", "B", "A", "S", "B", "A", "A+"]  # 代表性 7 年评级序列
    orig_total = sum(灵石奖励表[r] for r in 序列)
    # 分发后总量 = 每年来自 (年度占比 + 七载占比) 之和 = (annual+sept) × 原额
    dist_total = sum(灵石奖励表[r] * (annual + sept) for r in 序列)
    dev = (abs(dist_total - orig_total) / orig_total) if orig_total else 0.0
    if dev > 0.05:
        fail("7年总奖励偏差=%.2f%% > 5%%，通胀未锁死" % (dev * 100))
    annual_total = sum(灵石奖励表[r] * annual for r in 序列)
    pool_total = sum(灵石奖励表[r] * sept for r in 序列)
