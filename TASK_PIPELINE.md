...
#### 七、P2 社交联动（2026-09-12 晚 · 老大指令「再开 P2」）

> **状态流转**：P0（21:55）→ P1（本日续）→ **P2 本轮完成** → **已 commit**。P3 未启动（任务书标「后续版本」）。

**3.2 排行榜联动** —— 修通一条**长期静默失效的「值」链**：`xuan_rank_system.gd::取玄榜()` 算完 `分` **未写入返回字典**，而排序 `_玄榜_按值降` 与 UI `page_leaderboard._建行` 都读 `"值"` → **排序恒失效 + 榜单全显示 0**；`取玄榜_self` 又硬取 `"境界值"`（切指标分数不变）。**一处补写 `"值": 分` 同时修好 4 处消费方**。两榜接真源：「探索」→ `世界地图系统.获取探索进度()`（已发现资源点×3 + 秘境×5 + 区域×10）×120；「财富」→ 补 `门派等级×50000` 基业；UI 宗门指标集补「财富」。

**3.1 社交联动** —— **消掉 2 个死函数**（死函数水位 391 → **389**）：`Game.与宗门结盟` / `Game.与宗门宣战` 原**全项目零调用方**，且大地图 20 个「其他宗门」与 `Game.宗门关系`（仅 6 个已知宗门）**完全不通**。新增 `world_map_system` 桥接：`_登记宗门外交`（实力越强初始好感越低）/ `获取遭遇宗门外交` / `与其他宗门问候`（每日限次；敌对反生嫌隙）/ `与其他宗门结盟` / `与其他宗门宣战` / `获取附近频道` / `获取同盟协防`。**「组队探索高危险区域」落点** = `弟子探索返回结算` 按同域友好宗门数给收获加成 `×(1+危险削减×0.5)`。UI：宗门详情补三行外交 + **传音问候 / 缔结盟约 / 下战书**；天下总览补**第九段「宗门交游」**（本域同盟 / 邻近宗门 / 附近频道）。

**持久化缺口（顺带发现并补）**：`宗门关系` **从未进 `to_dict/from_dict`** → 结盟与好感度**读档即丢**。已补 `"宗门外交.宗门关系"`（不升 `SAVE_VERSION`，旧档缺键→`{}` 后由 `_初始化宗门关系` 重新播种）。

**验证**：门禁 `gate_all.py` **EXIT 0**（门1 `PARSE OK (196 files)` / 门2 73 表·1701 行·0 错·56 警 / 门3 **32/32**）；全链 **7/7 exit=0，异常 0/7**（8m34s）；**真实渲染 8/8 截图 err=0 + SCRIPT ERROR 归零**，社交断言全通过（好感 35→39、冷却拒、好感不足拒结盟、宣战→敌对、附近频道 4 条含【敌雠】前缀）。

> ### ⚠ 本轮最重要的事：真实渲染验收**第二次救命**，且暴露门禁盲区
>
> 新代码 `_刷新天下总览()` 第九段写 `var 关: String`，而**同函数顶层已有 `var 关: Button`** → Godot `Parse Error: There is already a variable named "关" declared in this scope.` → **`ui/page_world_map_visual.gd` 整个脚本 load 失败 → 天下页整页空白**。
>
> **`gate_all.py` 当时全绿**（门1 报 `ALL GDScript PARSE OK (196 files)`），`_open_world_map_page()` **还打印了「已打开」** —— 它不校验实例化结果。
>
> **根因**：gdtoolkit **不做作用域分析**；且本项目唯一的全量防线 `headless_compile_all` **在 `run_headless_chain.py` 里、不在 `gate_all.py` 里** → **「gate_all 绿」≠「每个脚本都能编译」**。
>
...
## 【已完成】G 类 · 视觉一致性收口 · 第 1 批（图标超大）

**状态**：✅ 已完成（2026-09-13 凌晨） · **执行**：workbuddy（工程线） · **验收**：待老大复核
**来源**：**非管道任务** —— 老大直接反馈「好多页面的字体大小风格等都不统一，坊市里面的图标超大」+「整体风格还是旧的不太好看」+「这些放在什么时候修？」

### 结论先行：原计划有空白

| 老大反馈 | 原计划落点 | 结论 |
| --- | --- | --- |
| 字体大小不统一 | ❌ **无** | **计划空白** → 本轮立项 **G 类** |
| 坊市图标超大 | ⚠️ D2 是「豆包出图」，本项实为代码约束缺失 | 归属错位 → 工程线 |
| 整体风格旧 / 不好看 | ✅ A4 色彩回写 + A5 复兴色彩派生 + D2/D3 | 已排期，**未开工** |

> B 类字面就是「UI **布局**重构」：保证「每个功能有正确的位置、能看见、能点到」，**不保证好看**。
> 上轮汇报写「UI 重构完成」措辞不准，应更正为「**UI 布局重构（B 类）完成**」。

### 量化诊断（全项目只读扫描）

| 项 | 现状 | 规范 | 判定 |
| --- | --- | --- | --- |
| 字号裸写 | **442 处 / 20 种取值** | `ui_theme.gd` L161「全项目唯一来源，**禁止散写魔法数字**」，档位 60/48/45/33/27/21 | ❌ 100% 越档 |
| 图标尺寸约束 | 3 文件缺 `expand_mode`（4 处） | 另 21 文件均已设 | ❌ 本批已修 |
| 硬编码 `Color(...)` | **1446 次 / 76 文件** | 应走 `UITheme` const | ❌ 待收敛 |

**字号根因**：页面裸写的是「480×854 **设计基准 px**」，而 `UITheme.FONT_*` 是「**渲染 px**」，相差 `UI_SCALE = 2.25` —— **页面漏乘 UI_SCALE**。
交叉验证：`12×2.25 = 27 = FONT_BODY` ✓ ｜ `20×2.25 = 45 = FONT_TITLE` ✓。
→ 同一屏里走 `UITheme` 的标签比裸写的**大 2.25 倍**。

**收敛方案**（覆盖 99.5%）：10,11→`FONT_AUX 21`｜12,13→`FONT_AUX 21`｜14,15,16→`FONT_BODY 27`｜18,19,20→`FONT_H2 33`｜22,24→`FONT_TITLE 45`｜26~40→`FONT_DISPLAY 60`。

### 第 1 批落地：图标超大（根因 + 修复）

**根因**：Godot `TextureRect` 默认 `expand_mode = EXPAND_KEEP_SIZE`，此时 `get_minimum_size()` **返回贴图原始尺寸**，`custom_minimum_size` 仅下限。本项目图标是 512/72 级高清贴图 → minimum 被撑到 512 → **卡片撑爆、图标占半屏**。

| 文件 | 处 | 影响 |
| --- | --- | --- |
| `ui/page_shop.gd` | L149 货币胶囊 | 超出 16px 设计 |
| `ui/page_shop.gd` | L629 商品图标 | ★ 坊市列表整页被撑爆（一屏仅 1 件） |
| `ui/collapsible_category.gd` | L45 折叠 chevron | 超出 24px 设计 |
| `ui/page_chronicle.gd` | L79 页头图标 | `(24,24)` 设计实显 72（3 倍） |

**修法**：补 `expand_mode = TextureRect.EXPAND_IGNORE_SIZE`（全项目另 21 文件的标准写法）。

**验证**：门禁 `EXIT 0`；全量真实渲染 **64 页** `badscroll=overflow=offscreen=collapse=0`；坊市一屏 **1 件 → 10 件**、9 个 tab 全可见（对比图 `accept_fangshi_before_after.png`，SHA256 校验回退/还原无损）。

### 后续（G 类余项）

| # | 任务 | 规模 | 前置 |
| --- | --- | --- | --- |
| G2 | 字号收敛 442 处 → 6 档 | 442 处 | **先出 2–3 页打样给老大认可** |
| G3 | 硬编码色 → 令牌 | 1446 处 / 76 文件 | G2 后 |
| G4 | 规范回写 `UX设计总纲 §4.1` | 文档 | G2 定稿后 |
| G5 | 图标显示尺寸分级落到 62 处 TextureRect | 62 处 | G1 后 |
| G6 | 成套 UI 皮肤 | — | **A4+A5+D2/D3 全落地后** |
