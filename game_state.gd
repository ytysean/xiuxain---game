...
var 捐赠记录: Dictionary = {}          # 图鉴ID -> true（稀有藏品已捐藏宝库）
var 传承史册: Array = []               # 持久化史册（图鉴集齐 + 里程碑，复用 #2 纪事结构；独立于会话瞬时 宗门纪事）
...
# WAVE-D #6/#8：图鉴/里程碑变化 → main.gd 刷新二级页（B2 次页，不新增 Tab）
signal 图鉴更新
signal 里程碑更新
...
func _ready():
	load_game()
	_加载图鉴配置()      # WAVE-D #6（须在 _记录图鉴/_传承事件 之前）
	_加载里程碑配置()    # WAVE-D #8（须在 _传承事件/_复检里程碑 之前）
	if 弟子列表.is_empty():
		初始建宗()
	...
	_加载阵法配置_S1()
	_加载阵法物品_S1()
	_复检里程碑()            # WAVE-D #8：load/新局后按当前状态补达成阈值/状态里程碑
	弟子变动.emit()

# WAVE-B #2：取弟子纪事（按 弟子ID 过滤；旧档 entry 缺 弟子ID 时回退 弟子==姓名，兼容旧档）
func 取弟子纪事(目标ID: int, 姓名: String = "") -> Array:
	var 结果: Array = []
	for 记 in 宗门纪事:
		var 记ID: int = int(记.get("弟子ID", 0))
		if 记ID != 0:
			if 记ID == 目标ID:
				结果.append(记)
...
# WAVE-C #7：集市激活时数量翻倍（商品倍率）+ 稀有商队标记（稀有权重）；仅刷新规则，零经济（价格/限购通道不变）
func 刷新坊市上架():
	var 全部: Array = _坊市表()
	坊市稀有标记.clear()
	...
	var 基础数: int = randi_range(8, 12)
	var n: int = mini(int(round(基础数 * 倍率)), 全部.size())
	坊市上架集 = []
	全部.shuffle()
	for i in n:
	...
	if 激活 and 坊市上架集.size() > 0:
		...
		var 池: Array = 坊市上架集.duplicate()
		池.shuffle()
		for i in 标数:
			坊市稀有标记[池[i]] = true
	坊市上架_集市标记 = 激活

# WAVE-C #7：宗门集市——配置懒加载 / 激活判定 / 日推进钩子（仅刷新规则，零经济）
func 宗门集市_配置() -> Dictionary:
	if not 宗门集市配置缓存.is_empty():
		return 宗门集市配置缓存
	# 默认（与 GDD §7.4 一致；缺 CSV / 解析失败→常量回退，No-op 安全）
	var 默认: Dictionary = {
	...
	if not 行.is_empty():
		...
		默认["商品倍率"] = float(行.get("商品倍率", 默认["商品倍率"]))
		默认["稀有权重"] = float(行.get("稀有权重", 默认["稀有权重"]))
		默认["手续费系数"] = float(行.get("手续费系数", 默认["手续费系数"]))
		默认["声望系数"] = float(行.get("声望系数", 默认["声望系数"]))
	宗门集市配置缓存 = 默认
	return 宗门集市配置缓存

# WAVE-C #7：宗门集市是否激活（基准日默认=当前累计游戏日；月内日 = 日 % 30，窗口 [激活日, 激活日+持续)）
...
# WAVE-C #7：日推进钩子——进入/退出窗口时强制刷新坊市上架（仅刷新规则；刷新为替换语义，不会双倍上架，与周刷新天然去重）
func _宗门集市_日推进(日: int):
	var 前激活: bool = 宗门集市_激活中(累计游戏日 - 日)
	var 后激活: bool = 宗门集市_激活中(累计游戏日)
	if 后激活 and not 前激活:
		刷新坊市上架()   # 进入窗口：刷新为集市商品（数量翻倍+稀有商队）
...
# WAVE-D #6：加载 收藏图鉴分类.csv（plain UTF-8 无 BOM；DestinyDataLoader 不剥 BOM，故无 BOM）
func _加载图鉴配置():
	图鉴配置 = DestinyDataLoader._read_csv("res://config/收藏图鉴分类.csv")
	for r in 图鉴配置:
		var 类别: String = r.get("类别", "")
		if 类别 != "" and not 收藏图鉴_已收集.has(类别):
			收藏图鉴_已收集[类别] = []

# WAVE-D #8：加载 宗门里程碑.csv（触发类型: 事件/阈值/状态）
func _加载里程碑配置():
	里程碑配置 = DestinyDataLoader._read_csv("res://config/宗门里程碑.csv")
	for m in 里程碑配置:
		m["已达成"] = 里程碑_已达成.has(m.get("里程碑ID", ""))
...
func _七载大考():
	...
	宗门纪事.append({"日": 累计游戏日, "弟子": "", "稀有度": "宗门", "名称": "七载大典",
		"文案": "第%d七载，太玄宗七载大典礼成，普天同庆，万修来朝，宗门威仪更盛。" % 序号,
		"category": "大典盛事"})
	if 晋升 > 0:
...
func _尝试触发奇遇(d: Disciple, scene: String, 保底: bool = false) -> Dictionary:
	...
	# 宗门纪事（最简版）：追加本次触发记录
	宗门纪事.append({
		"日": 累计游戏日, "弟子": d.姓名, "弟子ID": d.弟子ID, "稀有度": q.get("稀有度", "普通"),
...
func _掉落转物品(drop: Dictionary) -> Item:
	...
	_记录图鉴(it.名称, "掉落")   # WAVE-D #6：掉落即收录图鉴（阵法/天材/灵草 匹配名 = 物品名）
	return it

# ============ WAVE-D #6 宗门收藏图鉴 ============
# 首次获得即收录（来源：掉落物品 / 御兽孵化 / 传承事件）。匹配名 = 物品名 / 灵兽种类名 / 事件键。
# 零经济改动：只读 图鉴配置 与 收藏图鉴_已收集。
func _记录图鉴(匹配名: String, 来源 := "") -> bool:
	if 匹配名 == "":
		return false
	var 新收录 := false
	for r in 图鉴配置:
		if r.get("匹配名", "") == 匹配名:
			var 类别: String = r.get("类别", "")
			if not 收藏图鉴_已收集.has(类别):
				收藏图鉴_已收集[类别] = []
			if not 收藏图鉴_已收集[类别].has(匹配名):
				收藏图鉴_已收集[类别].append(匹配名)
				新收录 = true
				_检查图鉴类别集齐(类别)
	if 新收录:
		图鉴更新.emit()
...
# 某类别全部 匹配名 是否已收录；集齐则解锁 产出池轴 增益（单条 ≤5%，全宗 ≤30% via clamp 兜底）
func _检查图鉴类别集齐(类别: String):
	var 全部: Array = []
	var 增益值: float = 0.0
	for r in 图鉴配置:
		if r.get("类别", "") == 类别:
	...
	传承史册.append({"日": 累计游戏日, "弟子": "", "名称": "图鉴集齐", "文案": "【图鉴】%s 已集齐，宗门产出运势小成（产出 +%d%%）。" % [类别, int(增益值 * 100)], "category": "传承"})
	里程碑更新.emit()

# ============ WAVE-D #8 宗门里程碑 / 传承树 ============
# 传承事件 → 先贤事迹图鉴收录 + 事件型里程碑达成（键同源）
func _传承事件(键: String):
	_记录图鉴(键, "事件")
	for m in 里程碑配置:
		if not m.get("已达成", false) and m.get("触发类型", "") == "事件" and m.get("触发事件", "") == 键:
			_达成里程碑(m)

# 按当前状态复检 阈值/状态 型里程碑（不调用 _加声望，避免声望里程碑递归）
func _复检里程碑():
	for m in 里程碑配置:
		if m.get("已达成", false):
			continue
		var t: String = m.get("触发类型", "")
		var k: String = m.get("触发事件", "")
		var v: float = float(m.get("触发阈值", 0))
		var 达成 := false
		if t == "阈值":
			match k:
				"声望": 达成 = 声望 >= v
...
func _达成里程碑(m: Dictionary):
	...
	if 类型 == "产出池":
		...
		_加声望(int(值))   # 复用现有 声望 变量与乘区；绝不落 凝聚力（16 审计 ❌ 零运行时）
	_记录图鉴(m.get("名称", ""))   # WAVE-D D-2/F1：里程碑 名称 与先贤事迹图鉴 匹配名 同构，达成时一并收录，使六类图鉴全可达
	传承史册.append({"日": 累计游戏日, "弟子": "", "名称": "里程碑", "文案": m.get("史册文案", ""), "category": "传承"})
	里程碑更新.emit()
...
# WAVE-D #6：稀有藏品捐赠藏宝库 → 声望（复用现有 声望，不落凝聚力）
func 捐赠图鉴(图鉴ID: String) -> Dictionary:
...
func _晋升仪式感(d: Disciple):
	宗门纪事.append({"日": 累计游戏日, "弟子": d.姓名, "弟子ID": d.弟子ID, "稀有度": "宗门", "名称": "阶位晋升", "文案": "【晋升】%s 晋升为 阶位·%s" % [d.姓名, d.阶位]})
...
func 罢免阶位(d: Disciple) -> Dictionary:
	...
	结果["ok"] = true
	宗门纪事.append({"日": 累计游戏日, "弟子": d.姓名, "弟子ID": d.弟子ID, "稀有度": "宗门", "名称": "罢免阶位", "文案": "【罢免】%s 被罢去阶位，现任 阶位·%s。" % [d.姓名, d.阶位]})
	return 结果
...
func 举办测灵根(强制天品: bool = false) -> Dictionary:
	...
	for 徒 in 新徒:
		if 徒.灵根品阶 == "天品":
			宗门纪事.append({"日": 累计游戏日, "弟子": 徒.姓名, "弟子ID": 徒.弟子ID, "稀有度": "天品", "名称": "天品灵根弟子", "文案": "接引大典检出天品灵根弟子【%s】，全宗士气大振。" % 徒.姓名})
...
func 推进孵化(日数: int):
	...
	for e in 已孵:
		灵兽蛋列表.erase(e)
		灵兽库存.append(e)
		_记录图鉴(e.种类名, "御兽")   # WAVE-D #6：孵化即收录妖兽图鉴（匹配名 = 灵兽种类名）
	if 已孵.size() > 0:
...
func 处理坐化(名单: Array[Disciple]):
	...
	for d in 名单:
		...
		宗门纪事.append({"日": 累计游戏日, "弟子": d.姓名, "弟子ID": d.弟子ID, "名称": "坐化", "文案": _坐化纪事文案(d), "category": "先贤缅怀"})
		_加推演条目("【%s】寿元耗尽，坐化于山门。" % d.姓名, ET_SECT, PRIO_NORMAL, {"弟子": d.姓名})
		if d.身份 == "长老":
			_传承事件("首位长老飞升")   # WAVE-D #8：长老坐化 → 先贤事迹图鉴 + 事件型里程碑
		_复检里程碑()                      # WAVE-D #8：弟子/长老状态变化复检
		弟子列表.erase(d)
...
func load_game():
	...
	先贤堂 = data.get("先贤堂", [])
	# WAVE-D #6/#8：图鉴/里程碑/传承史册（旧档缺键→默认零回归）
	收藏图鉴_已收集 = data.get("图鉴_已收集", {})
	产出池加成 = data.get("产出池加成", 0.0)
	里程碑_已达成 = data.get("里程碑_已达成", [])
	捐赠记录 = data.get("捐赠记录", {})
	传承史册 = data.get("传承史册", [])
