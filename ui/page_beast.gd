...
func _build() -> void:
	...
	for 标签名 in TABS:
		...
		按钮.text = 标签名
		按钮.custom_minimum_size = Vector2(90, 32)
...
func _填总览() -> void:
	...
	_添加信息项(坐骑网格, "总战力加成", "+%d" % int(坐骑统计.get("总战力加成", 0)))
...
func _填灵兽库() -> void:
	...
	if 灵兽列表.is_empty():
		_添加空状态("暂无灵兽", "灵兽库存为空，可通过引育计划或历练获取灵兽。")
		return
	...
	for i in range(灵兽列表.size()):
		...
		等级标签.text = "等级：%d/%d" % [int(灵兽.get("等级", 0)), int(灵兽.get("等级上限", 0))]
		...
		战力标签.text = "战力：%d" % int(灵兽.get("战力", 0))
...
func _填出战绑定() -> void:
	...
	for d in Game.弟子列表:
		...
		if d.主宠灵兽 != null or d.副宠灵兽 != null:
			...
			if d.主宠灵兽 != null:
				...
				主宠标签.text = "  主宠：%s（Lv.%d，战力%d）" % [d.主宠灵兽.种类名, d.主宠灵兽.等级, d.主宠灵兽.本体战力()]
			...
			if d.副宠灵兽 != null:
				...
				副宠标签.text = "  副宠：%s（Lv.%d，战力%d）" % [d.副宠灵兽.种类名, d.副宠灵兽.等级, d.副宠灵兽.本体战力()]
	...
	if not 有出战弟子:
		_添加空状态("暂无出战灵兽", "目前没有弟子绑定灵兽，可在「灵兽库」中查看库存灵兽。")
	...
	for 兽 in Game.灵兽管理系统.灵兽库存:
		if 兽 != null and not 兽.孵化中:
			...
			var 名: Label = Label.new()
			名.text = "%s（Lv.%d，战力%d）" % [兽.种类名, 兽.等级, 兽.本体战力()]
...
func _填引育计划() -> void:
	...
	if 队列.is_empty():
		_添加空状态("暂无引育计划", "可添加引育计划，自动拨付灵石引育灵兽。")
	else:
...
func _填坐骑() -> void:
	...
	加成标签.text = "  速度+%.0f%%，战力+%d" % [float(当前加成.get("速度加成", 0)) * 100, int(当前加成.get("战力加成", 0))]
	...
	for 坐骑 in 坐骑列表:
		...
		var 战力标签: Label = Label.new()
		战力标签.text = "战力+%d" % int(坐骑.get("战力加成", 0))
...
func _激活坐骑(坐骑ID: String) -> void:
	...
	_提示 = str(结果.get("消息", 结果.get("原因", "操作完成")))
	_刷新内容()
...
func _填繁殖() -> void:
	...
	if 可繁殖.size() < 2:
		_添加空状态("无可繁殖灵兽", "需要至少2只亲密度>=20的契约灵兽才能繁殖")
		return
	...
	for i in range(min(4, 可繁殖.size())):
		...
		var 名称标签: Label = Label.new()
		名称标签.text = "%s（%s，战力%d，亲密度%d）" % [str(灵兽.get("名称", "")), str(灵兽.get("境界", "")), int(灵兽.get("战力", 0)), int(灵兽.get("亲密度", 0))]
