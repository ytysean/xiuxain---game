...
# 美术接口已预留：测灵根过场/殿阁面板/弟子卡均可后续替换为真实立绘与动画节点。
extends Control
...
# 暗化叠层（§3.5，入门指引/收尾，原 Color(0.06,0.06,0.08,X) 收口）
const 暗化 := Color(0.06, 0.06, 0.08)
# 墨青底（§3.5，入门指引气泡/收尾气泡，原 Color(0.118,0.169,0.157,0.96) 收口）
...
const 殿阁彩蛋阈值 := {"dantang": 5, "yushou": 5, "lingtian": 3}
var 殿阁点击计数: Dictionary = {}
var 掌教点击计数: int = 0
...
# 职能标签（小字次要灰，按殿阁取；复用差事示例词 核心生产/防御增益/人才入口）
const 殿阁职能标签 := {
...
# 御兽/灵兽节点（R8/Q8，完整UX设计规范.md §2.5/§9）：御兽已不属底部 Tab，归 殿阁→hall_yushou（S1 落地）。
# 本节点与 _建_御兽页()/_刷新御兽() 保留作殿阁子页复用，勿删（灵兽数据层已存在）。
...
var 历练滚动列: VBoxContainer = null  # 历练秘境列表（战斗胜利后需刷新）
# 新手入门指引系统（A 包：单线性五步，纯灰模）：阶段 0序章/1-5五步/6完成（Game.引导阶段 为事实源）
var 入门指引_殿阁按钮: Button = null
var 入门指引_招收按钮: Button = null
var 入门指引_历练按钮: Button = null
var 入门指引_层: Control = null          # 入门指引覆盖层（暗化+高亮框+气泡），最后 add_child 置顶，mouse_filter 穿透
var 入门指引_跳过按钮: Button = null     # 右上角常驻「跳过入门指引」，阶段<5 时可见
var 入门指引_开场段: int = 0             # 开场叙事当前段序号
var _入门指引已收尾: bool = false         # 收尾弹窗一次性标记（播完不再重复）
var _坊市提示: String = ""
var _差事提示: String = ""
# Step 2 奇遇基础调度：signal 入队、逐个弹窗，避免推演期多奇遇堆叠
var 奇遇队列: Array = []
var 奇遇弹窗中: bool = false
const 页名 := ["宗门", "弟子", "殿阁", "历练", "纪事"]
...
var 入门指引_目标条: PanelContainer = null
var 入门指引_目标条_文: Label = null
...
func _ready():
	...
	if ResourceLoader.exists("res://art/_references/ai_art/2026-07-22/仙山宗门远景_云雾半透明_古建木石_青黛色调_低饱和度_水墨_2026-07-22T15-19-56.png"):
		景.texture = load("res://art/_references/ai_art/2026-07-22/仙山宗门远景_云雾半透明_古建木石_青黛色调_低饱和度_水墨_2026-07-22T15-19-56.png")
...
func _初始化红点系统() -> void:
	...
	# ★ PH7-BATCH1E·任务5：SectManager 三信号（各带 3 参）此前全仓从未 connect ⇒ 若新增请命/违规
	#   走非月度推演路径，红点会静默滞后。SectManager 是独立 autoload，故单独成块（不塞进上面的 if Game）。
	if SectManager != null:
		SectManager.戒律违规上报.connect(_刷新红点, CONNECT_DEFERRED)
		SectManager.弟子请示.connect(_刷新红点, CONNECT_DEFERRED)
		SectManager.宗门事务待批阅.connect(_刷新红点, CONNECT_DEFERRED)

func _刷新红点(_a: Variant = null, _b: Variant = null, _c: Variant = null) -> void:
	# 统一吸收异参信号（CONNECT_DEFERRED 下参数数量不符会直接报错，用可选参数抹平）：
	#   · Game 四信号：弟子变动/新手目标更新/邮件变动 = 0 参；战报更新 = 1 参（文本）；
	#   · SectManager 三信号（PH7-BATCH1E·任务5 新增接线）：戒律违规上报/弟子请示/宗门事务待批阅 = 各 3 参。
	#   故签名加宽至 3 个可选参数，一处吸收全部。
	if _红点系统 != null and is_instance_valid(_红点系统):
		_红点系统.刷新所有红点()
...
func _on_主导航切换(i: int):
	...
	match 名:
		"宗门": _建_宗门页()
		"弟子": _建_弟子页()
		# R8/Q8（完整UX设计规范.md §2.5/§9）：御兽已不属底部 Tab，归 殿阁→hall_yushou（S1）。
		# 旧一级页分支 "_建_御兽页()" 已移除；_建_御兽页() 函数保留作殿阁子页复用，勿删。
		"历练": _建_历练页()
		"纪事": _建_纪事页()
	当前页名 = 名
...
func _建_弟子页():
	...
	接引框.add_child(抉择区)
	var 招 := Button.new(); 招.text = "举办测灵大典"; 招.name = "Button_开启接引大典"; 招.custom_minimum_size = Vector2(0, 44)
...
# 御兽页构建器（R8/Q8：御兽已不属底部 Tab，归 殿阁→hall_yushou（S1））。
# 现仅作殿阁子页预留复用入口，不再由底部主导航直接调用；保留玩法内容，勿删。
func _建_御兽页():
	var 页: ScrollContainer = _新页容器()
	var 内容: Control = 页.get_child(0)
...
func _建_弟子页头():
	...
	for 名 in ["默认", "道行", "境界", "资质"]:
		...
		排序行.add_child(钮)
	# S1 批1：批量试炼入口（弟子页顶部）
	var 批量试炼钮 := Button.new(); 批量试炼钮.text = "批量试炼"
	批量试炼钮.add_theme_font_size_override("font_size", FONT_AUX)
	批量试炼钮.add_theme_color_override("font_color", 暗金)
...
func _测灵根_执行():
	...
	if Game.引导阶段 == 4 and Game.弟子列表.is_empty():
		Game.引导阶段 = 3
	# 不在这里推进入门指引："收入外门"按钮才是步③的终点确认，点击后再推进到步④。
	# 否则弹窗还没关，步④气泡就会在遮罩后面指向弟子页，玩家无法交互。
	var r: Dictionary = Game.举办测灵根()
	if r.has("冷却剩余"):
		详情.text = "测灵大典尚在筹备中，筹备周期尚余 %d 天。" % r["冷却剩余"]
		return
	_过场弹窗(str(r["过场"]), int(r["人数"]))
	# 弹窗打开后刷新入门指引：目标从「举办测灵大典」切换为「收入外门」
	_入门指引_刷新()
...
func _弹七载大典():
	...
	if 灵石 > 0 or 法宝数 > 0 or 晋升至 > 0:
		...
		if 法宝数 > 0:
			...
			l.add_theme_font_size_override("font_size", FONT_AUX); l.add_theme_color_override("font_color", 次墨); 块.add_child(l)
		if 晋升至 > 0:
			var l := Label.new(); l.text = "〔宗门品阶跃迁〕晋至 %d 品" % 晋升至
			l.add_theme_font_size_override("font_size", FONT_AUX); l.add_theme_color_override("font_color", 次墨); 块.add_child(l)
...
func _弹出灵兽绑定选择(灵兽: Beast):
	...
	if 候选.is_empty():
		...
		for d in 候选:
			...
			信息.text = "%s · %s · 道行 %d · %s" % [d.姓名, d.境界, d.实时战力(), 标记]
			行.add_child(信息)
...
func _on_选弟子(d: Disciple):
	当前选中 = d
	详情.text = d.简介()
	# 入门指引步骤4：点开弟子详情后推进到完成（修复：之前误删导致卡死）
	if Game.引导阶段 == 4:
		_入门指引_推进("查看弟子")
...
func _on_道途(d: Disciple):
	var 职名: String = d.道途 if d.道途 != "" else "未入门"
	_弹出弟子属性弹窗("道途", "【道途·%s】\n%s" % [职名, d.道途详情()], d)

# ============ S1 批1：阶位轴操作（试炼晋升 / 罢免阶位 / 批量试炼）============
func _on_试炼晋升(d: Disciple):
	if d == null:
		return
	if not d.可授阶():
		...
		return
	if d.试炼冷却剩余 > 0:
		_toast("%s 试炼冷却中（剩余 %d 日）" % [d.姓名, d.试炼冷却剩余])
		return
	...
	if 占用 >= 上限:
		...
		return
	var 弹: Dictionary = _new_detail_popup("试炼晋升 · %s" % d.姓名)
	var 信息 := Label.new()
	信息.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	信息.text = "当前阶位：阶位·%s\n目标阶位：阶位·%s\n成功率：%d%%\n行政成本：贡献点 -%d（失败再 -%d）\n名额：%d/%d%s" % \
		[d.阶位 if d.阶位 != "无" else "无（未授阶）", 目标阶位, int(d.试炼成功率() * 100),
		 Game.试炼成本, Game.试炼失败扣减, 占用, 上限, ("（破格 +1 阶）" if 破格 else "")]
	弹["内容"].add_child(信息)
	var 行 := HBoxContainer.new(); 行.add_theme_constant_override("separation", 6)
	var 确认 := Button.new(); 确认.text = "发起试炼"
	确认.disabled = (Game.贡献点 < Game.试炼成本)
...
func _执行试炼(d: Disciple):
	var r: Dictionary = Game.发起试炼(d)
	if r.get("成功"):
		_toast("【晋升】%s 晋阶为 阶位·%s！" % [d.姓名, d.阶位])
	else:
		_toast("试炼结果：%s（%s）" % [("失利" if r.get("ok") else "未发起"), r.get("原因", "")])
	刷新()
...
func _弹出批量试炼():
	var 候选: Array = []
	for d in Game.弟子列表:
		if d.可授阶() and d.试炼冷却剩余 <= 0:
			候选.append(d)
	if 候选.is_empty():
		_toast("当前无可试炼弟子（须内门及以上且不在冷却中）")
		return
	var 弹: Dictionary = _new_detail_popup("批量试炼")
	var 说明 := Label.new()
	说明.text = "勾选参与本次试炼的弟子（逐人独立掷骰，独立判定冷却与名额）："
	说明.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	弹["内容"].add_child(说明)
	var 勾列表: Array = []
	for d in 候选:
		...
		行.add_child(勾)
		弹["内容"].add_child(行)
	var 确认 := Button.new(); 确认.text = "确认试炼"
	确认.pressed.connect(func():
		if not is_instance_valid(弹["遮"]):
			return
		var 选: Array = []
		...
		if 选.is_empty():
			_toast("未勾选任何弟子")
			return
		var 汇总: Dictionary = Game.批量试炼(选)
		_toast("批量试炼完成：成功 %d · 失败 %d · 跳过 %d · 贡献扣减 %d" % [汇总["成功"].size(), 汇总["失败"].size(), 汇总["跳过"].size(), 汇总["贡献扣减"]])
		刷新()
		if is_instance_valid(弹["遮"]):
			_关闭详情(弹["遮"])
	)
...
func _弹出功法详情(id: String) -> void:
	...
	vb.add_child(_属性行("适用道途：%s" % g.get("apply_class", "—"), 墨黑))
	vb.add_child(_属性行("功法类型：%s" % g.get("skill_type", "—"), 墨黑))
	vb.add_child(_属性行("效果数值：%s" % g.get("effect_value", "—"), 墨黑))
	vb.add_child(_属性行("最大参悟阶次：%s" % g.get("max_level", "—"), 墨黑))
...
func _弹出殿阁详情(key: String) -> void:
	var e: Variant = Game.司职列表.get(key)
	if e == null or not (e is Dictionary):
		_toast("（暂无详情）")
		return
	var 弹: Dictionary = _new_detail_popup(e.get("名称", "殿阁"), 青灰)
	var 滚 := ScrollContainer.new()
	滚.custom_minimum_size = Vector2(0, mini(340, int(get_viewport_rect().size.y * 0.7)))
	滚.size_flags_vertical = Control.SIZE_EXPAND_FILL
	弹["内容"].add_child(滚)
	var vb := VBoxContainer.new()
	滚.add_child(vb)
	vb.add_child(_属性行("司职key：%s" % key, 墨黑))
	vb.add_child(_属性行("品级：%s" % e.get("等级", "—"), 墨黑))
...
func _弹出灵兽详情(b: Beast) -> void:
	...
	vb.add_child(_属性行("品级：第%d重" % b.等级, 墨黑))
	vb.add_child(_属性行("品级上限：%d" % b.等级上限, 墨黑))
...
func 刷新御兽():
	...
	if Game.灵兽兑换队列.is_empty():
		var 空 := Label.new(); 空.text = "（未设置引育计划，轻触下方选项添加）"; 御兽区.add_child(空)
...
func 刷新纪事():
	...
	for 记 in Game.宗门纪事:
		...
		if 文案.contains("[url=") or 弟子名 != "":
			# S1 点击联动 MVP：含 bbcode 链接（奇遇赏赐物品）的纪事用 RichTextLabel 渲染，
...
func _过场弹窗(过场: String, 人数: int):
	...
	遮.name = "测灵根过场"
	add_child(遮)
	var pc: PanelContainer = 新面板("【测灵大典】")
	...
	内容.add_child(文)
	var 尾 := Label.new()
	尾.text = "本届测灵大典收录外门弟子 %d 人（本期测灵名额已用尽），潜心修行，他日或可证道。" % 人数
...
func _收入外门_关闭(遮: Node, 人数: int):
	# 关闭弹窗与推进入门指引必须全部延迟：「收入外门」按钮自身是弹窗子节点，
	# 同步操作会在 pressed 回调内释放发射者/重建页面，导致 freed instance 或 UI 重叠。
	详情.text = "测灵大典完成，录入 %d 名新弟子。" % 人数
	遮.call_deferred("queue_free")
	_入门指引_推进.call_deferred("招收")   # 推进到步④，由 _入门指引_刷新 统一刷新/重建页面

# ============ 殿阁总览弹窗（原「司职管理」；首发文案统一称殿阁，居中面板）============
# ============ 历练征途（Day 3 灰模 UI：秘境选择→上阵→战斗结算→掉落）============
# 架构解耦：UI 仅调用 Game.挑战秘境（数据驱动逻辑层），不碰 BattleCalculator/Manager 结算核心。
# ============ S0 FTUE 入门指引系统（五步，纯灰模，逻辑独立封装不侵入业务）============
# 阶段语义：0=序章中 / 1=资源栏 / 2=推演 / 3=招收 / 4=查看弟子 / 5=考评 / 6=完成(或跳过)
# 事实源：Game.引导阶段（game_state 存档，老档默认=6 不强制弹）。复用 GDD-新手入门指引 既有设定。

# 五步入门指引文案（修仙化包装「宗门传承指引」；气泡与目标条共用，目标条去前缀显示）
var 入门指引_步骤文案: Dictionary = {
	1: "【宗门传承指引】先戳一下顶上的家底——灵石、灵草、矿石、灵气，皆是宗门命脉。",
	2: "【宗门传承指引】查看离线简报 / 等待时辰流转——宗门自会运转，司职日积灵石灵草。",
	3: "【宗门传承指引】点「举办测灵大典」，收录第一名弟子入门，宗门方有传人。",
	4: "【宗门传承指引】到「弟子」页点开一人，看看他的境界、灵根与养成去处。",
	5: "【宗门传承指引】这「离山汇总」记着你离山期间宗门变化。宗门每岁末考评、每七载大考，考得越好，灵石越多。",
}
# 序章三屏（复用 GDD-新手入门指引 苏清禾/吴伯 设定；仅复用台词，不新增人设字段）
var 入门指引_开场文本: Array = [
	{"人":"【序章·继位】", "文":"老宗主已然仙去，临终把这座破落山门托付于你。太玄宗虽已式微，根基尚在——重振山门，就在此一举。"},
	{"人":"苏清禾", "文":"掌门莫慌，妾身会一路帮衬。咱宗门的家底就那几样：灵石、灵草、矿石、灵气。先认清它们，日后才好经营。"},
	{"人":"吴伯", "文":"山下青峰一带妖兽作乱，正可遣弟子去历练长进。不过不急——掌门先推演几日、收点产出、再开测灵大典广招门徒，宗门便转起来了。"},
]

func _入门指引_初始化():
	# 覆盖层：全屏、不拦截点击（点击穿透到下层真实按钮，强入门指引仅高亮不阻挡）
	if 入门指引_层 == null:
		入门指引_层 = Control.new()
		入门指引_层.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
		入门指引_层.mouse_filter = Control.MOUSE_FILTER_IGNORE
		入门指引_层.visible = false
		add_child(入门指引_层)
		if 入门指引_跳过按钮 == null:
			入门指引_跳过按钮 = Button.new()
			入门指引_跳过按钮.text = "跳过入门指引"
			入门指引_跳过按钮.add_theme_font_size_override("font_size", FONT_AUX)
			入门指引_跳过按钮.set_anchors_and_offsets_preset(Control.PRESET_TOP_RIGHT)
			入门指引_跳过按钮.offset_left = -84
			入门指引_跳过按钮.offset_top = 4
			入门指引_跳过按钮.offset_right = -8
			入门指引_跳过按钮.offset_bottom = 32
			入门指引_跳过按钮.visible = false
			入门指引_跳过按钮.pressed.connect(_入门指引_跳过)
			add_child(入门指引_跳过按钮)
	if 入门指引_目标条 == null:
		_新建目标条()
	入门指引_跳过按钮.visible = (Game.引导阶段 < 6)
	_刷新功能解锁()
	_刷新目标条()
...
func _入门指引_开场渲染():
	...
	var 内容: Control = pc.get_child(0)
	var 段: Dictionary = 入门指引_开场文本[入门指引_开场段]
	...
	var 末: bool = (入门指引_开场段 >= 入门指引_开场文本.size() - 1)
...
func _入门指引_刷新():
	...
	if 启用新UI:
		# ★ PH6-E1b·M1：新 UI 下旧 UI 目标控件（状态面板/标签栏/列表/离山面板）不存在，
		#   原 match 分支会触发旧 UI 导航（_on_主导航切换/刷新/弹离山简报）⇒ 崩溃。
		#   故新 UI 改为「居中气泡 + 知道了」自推进，不依赖任何旧 UI 节点（步骤 0→1→…→6 全可走通）。
		确认动作 = {"1": "资源", "2": "推演", "3": "招收", "4": "查看弟子", "5": "考评"}.get(str(Game.引导阶段), "")
		if 文案.strip_edges() != "":
			_入门指引_显示气泡.call_deferred(_入门指引_锚点(), 文案, 确认动作)
		_刷新功能解锁()
		_刷新目标条()
		return
	match Game.引导阶段:
		1: 目标 = 入门指引_状态面板
		2:
			目标 = 入门指引_状态面板   # 自动推演为信息步：无需点击按钮，附「知道了」确认推进
			确认动作 = "推演"
		3:
			...
			if 过场遮 != null and is_instance_valid(过场遮):
				...
				# 弹窗未打开：指向弟子页·接引 Tab 的「举办测灵大典」
				_弟子二级tab = 1
				_on_主导航切换(1)
				_应用_弟子二级tab()
				if not is_instance_valid(入门指引_招收按钮) and 当前页容器 != null:
					入门指引_招收按钮 = 当前页容器.find_child("Button_开启接引大典", true, false) as Button
				目标 = 入门指引_招收按钮
		4:
	...
	if 目标 != null and is_instance_valid(目标):
		if 文案.strip_edges() != "":   # 防护：空白文案不渲染气泡
			_入门指引_显示气泡.call_deferred(目标, 文案, 确认动作)
	elif Game.引导阶段 >= 6:
		入门指引_跳过按钮.visible = false
	_刷新功能解锁()
	_刷新目标条()

# M1p：新 UI 下按引导阶段解析需高亮的真实控件（Node.name + 运行时 rect，禁坐标）。
# 只切到承载页以确保控件已实例化；name 找不到一律返回 null ⇒ 调用方降级居中、不画框（绝不画错位框）。
func _入门指引_锚点() -> Control:
	if 新UI == null or not is_instance_valid(新UI):
		return null
	match Game.引导阶段:
		1:
			var 顶栏: Node = 新UI.find_child("TopBar", true, false)
			if 顶栏 == null:
				return null
			var 资源钮: Control = 顶栏.find_child("EntryBtn_灵石", true, false) as Control
			if 资源钮 == null:   # 降级：任一资源入口均可
				for c in 顶栏.get_children():
					if c is Control and String(c.name).begins_with("EntryBtn_"):
						资源钮 = c
						break
			return 资源钮
		2:
			return null   # 信息步（离线推演），无实体按钮 ⇒ 居中
		3:
			新UI.call("_show_page", "弟子")   # 先切承载页，否则控件未实例化
			# PH7-BATCH1D·步3 改锚：文案「举办测灵大典」是**主动触发动作** ⇒ 指向真实触发按钮
			#   ui/page_disciple.gd:224 CeilingBtn（「举办测灵大典」，连 _on_举办测灵根）；旧 UI 的
			#   Button_开启接引大典 在 启用新UI:=true(main.gd:52) 下永不执行。取不到 ⇒ null ⇒ 调用方降级居中，绝不画错位框。
			return 新UI.find_child("CeilingBtn", true, false) as Control
		4:
			新UI.call("_show_page", "弟子")
			return 新UI.find_child("Card_0", true, false) as Control
		5:
			新UI.call("_show_page", "宗门")
			var 主钮: Control = 新UI.find_child("MainCTA", true, false) as Control
			if 主钮 == null:
				主钮 = 新UI.find_child("SectHub", true, false) as Control
			return 主钮
	return null

func _入门指引_显示气泡(目标: Control, 文案: String, 确认动作: String = ""):
	if 文案.strip_edges() == "":
		return   # 防护：空白文案不渲染气泡（避免空白弹窗）
	# ★ PH6-E1b·M1：目标可为空（新 UI 无旧 UI 参照控件）⇒ 居中呈现气泡、不画高亮框。
	# M1p 降级加固：无目标 / 未入树 / 尚未布局（size≈0）一律居中，绝不画错位框。
	var 有目标: bool = is_instance_valid(目标) and 目标.is_inside_tree() and 目标.size.x >= 1.0 and 目标.size.y >= 1.0
	...
	入门指引_层.add_child(遮)
	# 高亮框（透明底 + 暗金边框圈住目标，色收敛自赛博青霓）—— 仅在有目标时绘制
	var 框位置: Vector2 = Vector2.ZERO
	var 框尺寸: Vector2 = Vector2.ZERO
	if 有目标:
		var 框 := Panel.new()
		...
		var lp: Vector2 = 入门指引_层.get_global_position()
		框位置 = gp - lp
		框尺寸 = 目标.size
		框.position = 框位置
		框.size = 框尺寸
	...
	if 有目标:
		泡.position = Vector2(框位置.x, 框位置.y + 框尺寸.y + 6)
		if 泡.position.y + 80 > 854:
			泡.position.y = max(4.0, 框位置.y - 90.0)
	else:
		# 居中（新 UI：无参照控件，居中呈现避免气泡贴边被裁）
		泡.anchor_left = 0.5
		泡.anchor_top = 0.5
		泡.anchor_right = 0.5
		泡.anchor_bottom = 0.5
	...
	if 确认动作 != "":
		...
		知.pressed.connect(func(): _入门指引_推进(确认动作))
		泡.add_child(知)
	入门指引_层.add_child(泡)
	入门指引_层.visible = true
...
func _跳转(路: String):
	...
	match 页:
		...
		"历练": _on_主导航切换(3)
		"纪事": _on_主导航切换(4)
		# 御兽已不属底部 Tab（R8），归 殿阁→hall_yushou（S1）；旧跳转统一导向殿阁(index=2)
		"御兽": _on_主导航切换(2)
		_: _on_主导航切换(0)
...
func _入门指引_跳过():
	...
	if 遮 != null and is_instance_valid(遮):
		_关_离山简报()
	if 入门指引_跳过按钮 != null and is_instance_valid(入门指引_跳过按钮):
		入门指引_跳过按钮.visible = false
	_刷新功能解锁()
	_刷新目标条()
	_清除招徒高亮()
...
func _显示标题():
	...
	_装衬线字体(主标题, 64, 12)
	主标题.add_theme_color_override("font_color", UITheme.color_text_gold())
	主标题.add_theme_color_override("font_outline_color", Color(UITheme.获取背景色(), 0.55))
	主标题.add_theme_constant_override("outline_size", 8)
	主标题.add_theme_constant_override("shadow_offset_x", 0)
	主标题.add_theme_constant_override("shadow_offset_y", 3)
	主标题.add_theme_color_override("font_shadow_color", Color(UITheme.获取背景色(), 0.45))
	...
	副标题.offset_bottom = title_y + 119.0
	_装衬线字体(副标题, 30, 8)
	副标题.add_theme_color_override("font_color", Color(UITheme.color_text_gold(), 0.82))
	副标题.add_theme_constant_override("shadow_offset_x", 0)
	副标题.add_theme_constant_override("shadow_offset_y", 1)
	副标题.add_theme_color_override("font_shadow_color", Color(UITheme.获取背景色(), 0.30))
	入门指引_标题层.add_child(副标题)

	# 云纹装饰线（复用项目自带云纹细线，与开屏页统一审美）
	var 金线: Control = UITheme.make_divider_control()
	金线.name = "TitleLine"
	金线.custom_minimum_size = Vector2(340.0, 4.0)
	金线.offset_left = 190.0
	金线.offset_top = title_y + 134.0
	金线.offset_right = 530.0
	金线.offset_bottom = title_y + 138.0
	金线.modulate = Color(1.30, 1.22, 1.10, 1.0)
	...
	标语.add_theme_color_override("font_color", UITheme.color_text_body_dim())
	标语.add_theme_constant_override("shadow_offset_x", 0)
	标语.add_theme_constant_override("shadow_offset_y", 1)
	标语.add_theme_color_override("font_shadow_color", Color(UITheme.获取背景色(), 0.30))
	...
	版本.add_theme_color_override("font_color", Color(UITheme.color_text_body_dim(), 0.55))
...
func _建_登录面板() -> void:
	...
	if 账号们.is_empty():
		...
		空.text = "尚无道号，轻触下方「新建道号」开启你的第一个宗门。"
		...
		for acc in 账号们:
			...
			卡.add_theme_stylebox_override("panel", _暗墨卡片())
			...
			删.add_theme_color_override("font_color", Color(UITheme.color_text_gold(), 0.75))
			行.add_child(进)
			行.add_child(删)
			卡内.add_child(行)
			列表.add_child(卡)
...
func _标题_关闭():
	if 入门指引_标题层 != null and is_instance_valid(入门指引_标题层):
		入门指引_标题层.queue_free()
		入门指引_标题层 = null
...
# 接收创建宗门页信号：写库（宗门名/宗主名/性别/三层外观 idx），进入经营主页
# 2026-09-15：不再给任何弟子冠上宗主名（宗主=独立实体 Game.宗主，不占弟子名额）
func _on_创建完成(宗门名: String, 宗主名: String, 性别: String, 头像id: String) -> void:
	Game.宗门名 = 宗门名
	Game.宗主名 = 宗主名
	Game.宗主性别 = 性别
	Game.宗主头像 = 头像id
	# 2026-09-15 修：此处原有一段「把第一个筑基弟子改名成宗主名」的冠名逻辑，
	#   导致弟子列表里永久多出一个与玩家同名者（现象：新弟子必有一个叫宗主名）。
	#   宗主已有独立实体 Game.宗主（见 game_state.gd _初始化宗主()），无需再占用弟子名额。
	#   旧存档中已被冠名的弟子，由 load_game 里的 _净化弟子与宗主同名() 一次性改名。
...
func _进入主界面():
	...
	#   · 宗门事件/违规/请示/批阅不产生
	#   —— 即「全自动」这一核心设定在实机是停摆的。
	# E 阶段「进入」卡顿异步化：推演前盖遮罩，await 期间主线程让帧，避免长冻黑屏
	var 遮: Control = _建进入遮罩()
	add_child(遮)
	_离线推演报 = await Game.推演至现在()
	遮.queue_free()
	# FTUE 保底（PH6·M4）：新档真实流逝≈0，保底推进 1 游戏日，确保入门指引有正反馈。
	# 原条件含 `引导阶段>=1`，而新档开机 `引导阶段==0`（序章尚未点「开始治宗」）⇒ 恒不触发（A8 断点）。
	# 改判只看 `累计游戏日==0`（与引导阶段解耦），让开机即有第一次「我动了、宗门动了」的正反馈。
	if Game.累计游戏日 == 0:
		Game.推演一月(1)
	_启动时间心跳()   # 解耦：在线期间游戏时间按真实时钟自然流逝
	# ★ PH6-E1b·M2 / PH7-BATCH1C·M2r（方向修正）：新手目标链与 FTUE 收尾/跳过解耦。
	# 原实现仅在 _入门指引_收尾/跳过（旧 UI 分支）调用 ⇒ 新 UI 下引导阶段恒为 0 ⇒ 目标链永不激活、
	# 「宗门_宗务」红点恒不亮。改挂本公共入口（新档/读档/登录三路共用）；Game.新手目标链激活 持久化且幂等。
	# 方向：确为「引导阶段 >= 6」（FTUE/序章 收尾或跳过）时才解锁，与 _入门指引_收尾/跳过 的
	#   「引导阶段=6 + 激活新手目标链」语义一致（game_state.gd:3537「FTUE 收尾后解锁」）。
	#   Batch 1B 曾误作 `< 6` 并附「过引导期即熄灯」elif —— 收尾后激活会被下一句当场熄灭，属净回归，
	#   该 elif 已整段删除。老档一次熄灯迁移改在 load 路径完成（game_state.gd:25623），此处只幂等激活。
	if Game.引导阶段 >= 6:
		Game.激活新手目标链()
	if 启用新UI:
		...
		# ★ PH6-E1b·M1：把 S0 FTUE 接进新 UI。原实现只在旧 UI 分支（函数末尾）调 _入门指引_初始化()，
		#   新 UI 分支提前 return ⇒ 序章/常驻目标条/气泡在实机永不出现（新玩家首日体验为零）。
		#   此处补调：入门指引_层 与 目标条 作为 main 子节点、晚于 新UI 挂载 ⇒ 天然位于新 UI 之上。
		_入门指引_初始化()
		return
...
func _建进入遮罩() -> Control:
	# E 阶段「进入」遮罩：推演离线结算期间覆盖全屏，避免长冻黑屏（复用弹窗遮罩配色）
	var c := Control.new()
	c.name = "进入遮罩"
	...
	var 字 := Label.new()
	字.text = "宗门推演中……"
	字.add_theme_font_size_override("font_size", 30)
	字.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	字.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	字.offset_top = -24
	c.add_child(字)
	return c
...
# 梯度（主理人裁定）：坊市 FTUE 后立开；差事 Lv>=2；历练 主条件（御兽已不属底部 Tab，归 殿阁→hall_yushou，见 R8/Q8）
func _功能解锁(名: String) -> bool:
	match 名:
		"坊市": return Game.引导阶段 >= 6 or Game.门派等级 >= 3
		"差事": return Game.门派等级 >= 2 or Game.引导阶段 >= 6
		"历练": return Game.引导阶段 >= 6 or Game.门派等级 >= 3
		_: return Game.引导阶段 >= 6 or Game.门派等级 >= 3

func _灰锁提示(名: String) -> String:
	match 名:
		"坊市": return "完成新手入门指引后开启坊市"
		"差事": return "宗门 2 品 后开启师门事务"
		"历练": return "宗门 3 品 或完成入门指引后开启历练"
		_: return "后续开启"

func _刷新功能解锁():
	# 二级页受锁入口（坊市/差事）：disabled+◇+tooltip，点击不跳转
	# 关键修复：页面切换时（如 FTUE 切到弟子页）宗门网格按钮会被释放，入门指引_坊市按钮/差事按钮
	# 会变成 dangling freed ref；直接强类型 var b: Button = 规则[名] 会触发"assign invalid previously freed instance"。
	# 改用条件构建+无类型变量，只操作仍存活的实例。
	var 规则: Dictionary = {}
	if is_instance_valid(入门指引_坊市按钮):
		规则["坊市"] = 入门指引_坊市按钮
	if is_instance_valid(入门指引_差事按钮):
		规则["差事"] = 入门指引_差事按钮
	for 名 in 规则:
		...
		if 开:
			...
			b.modulate.a = 0.5
	# 受锁核心 Tab（历练）：disabled + ◇ 文字 + tooltip，置灰可显（§1.5）。
	# 御兽已不属底部 Tab（R8/Q8，完整UX设计规范.md §2.5/§9），归 殿阁→hall_yushou（S1）；故不再作受锁核心 Tab。
	if 标签栏 != null:
		for 名 in ["历练"]:
...
func _刷新目标条():
	...
	if Game.引导阶段 < 1 or Game.引导阶段 > 5:
		入门指引_目标条.visible = false
		return
	入门指引_目标条.visible = true
	var 步: int = Game.引导阶段
	var 文案: String = 入门指引_步骤文案.get(步, "")
	if 文案.begins_with("【宗门传承指引】"):
		文案 = 文案.trim_prefix("【宗门传承指引】")   # 目标条文案去前缀
	入门指引_目标条_文.text = "当前目标 · 第 %d/5 步：%s" % [步, 文案]

func _新建目标条():
	入门指引_目标条 = PanelContainer.new()
	入门指引_目标条.anchor_left = 0.0
	入门指引_目标条.anchor_top = 0.0
	入门指引_目标条.anchor_right = 1.0
	入门指引_目标条.anchor_bottom = 0.0
	入门指引_目标条.offset_left = 8
	入门指引_目标条.offset_top = 4
	入门指引_目标条.offset_right = -92
	入门指引_目标条.offset_bottom = 48
	入门指引_目标条.custom_minimum_size = Vector2(0, 44)   # 点击区≥44px
	var sb := StyleBoxFlat.new()
	sb.bg_color = 墨底半透
	sb.border_color = 暗金
	sb.set_border_width_all(1)
	sb.set_corner_radius_all(6)
	入门指引_目标条.add_theme_stylebox_override("panel", sb)
	var 内 := VBoxContainer.new()
	入门指引_目标条_文 = Label.new()
	入门指引_目标条_文.add_theme_font_size_override("font_size", FONT_BODY)
	入门指引_目标条_文.add_theme_color_override("font_color", 宣纸亮)
	入门指引_目标条_文.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	内.add_child(入门指引_目标条_文)
	入门指引_目标条.add_child(内)
	入门指引_目标条.visible = false
	add_child(入门指引_目标条)

func _招徒后高亮弟子入口():
	_招徒后_弟子高亮中 = true
	# 仅当当前不在弟子页时才切页（避免入门指引刷新已切页后重复重建，也隔绝 pressed 回调内重建风险）
	if 当前页名 != "弟子":
		_on_主导航切换(1)
	if 弟子页头 != null and is_instance_valid(弟子页头):
		弟子页头.modulate = 暗金   # 高亮弟子页头
	if 入门指引_弟子提示 != null and is_instance_valid(入门指引_弟子提示):
...
func _填_坊市页(内容: Control):
	...
	if Game.宗门集市_激活中():
		...
		状文.add_child(状标)
		var 状描 := Label.new()
		状描.text = "商品数量×%s；稀有商队入驻；手续费系数 %s、声望系数 %s（集市缘法·显示态）" % [配.get("商品倍率", 2.0), 配.get("手续费系数", 0.9), 配.get("声望系数", 1.2)]
...
func _刷新坊市列表(列: VBoxContainer, 结果: Label):
	...
	for r in Game._坊市表():
		...
		var 折扣说明: String = "" if 折后 == 原价 else "（%s让利%d%%）" % [等级名, int(round((1.0 - Game.坊市折扣率()) * 100))]
...
func _刷新_差事页(结果: Label):
	...
	if Game.当前周常.is_empty():
		var 无 := Label.new(); 无.text = "（暂未解锁，提升宗门品级后开启）"; 差事列.add_child(无)
	else:
...
func _历练秘境卡(s: Dictionary) -> Control:
	var pc: PanelContainer = 新面板("")
	var 内容 := VBoxContainer.new()
	pc.add_child(内容)
	...
	var 已通: bool = Game.已通关秘境.has(sid)
	var 头 := Label.new()
	头.text = "%s %s%s" % [sid, s.get("stage_name", ""), "  ✓首通" if 已通 else ""]
	内容.add_child(头)
	var 副 := Label.new()
	副.text = "%s ｜ 气力%d ｜ 推荐道行%s%s" % [类型名, int(s.get("stamina_cost", "0")), str(StageDataLoader.get_recommend_power(s.get("stage_id", ""))), " ｜ 未解锁" if not 解锁 else ""]
	副.add_theme_color_override("font_color", 次墨)
...
func _开战弹窗(s: Dictionary):
	...
	内容.add_child(头)
	var 副 := Label.new()
	副.text = "气力-%d ｜ 推荐道行%s ｜ 最多3名弟子" % [int(s.get("stamina_cost", "0")), str(StageDataLoader.get_recommend_power(s.get("stage_id", "")))]
	副.add_theme_color_override("font_color", 次墨)
	内容.add_child(副)
	var 选中: Array = []
	var 状态标 := Label.new()
...
func _战斗结算面板(战报: Dictionary, stage_id: String):
	...
	# 入门指引推进/收尾移到"确定"按钮回调（结算遮罩销毁后再弹，避免被盖住）
	var 遮 := ColorRect.new()
	遮.color = 墨底
	遮.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	遮.mouse_filter = Control.MOUSE_FILTER_STOP
	...
	内容.add_child(回合)
	var 赏赐 := Label.new()
	赏赐.text = "赏赐：%s" % (战报.get("赏赐摘要", "") if 战报.get("赏赐摘要", "") != "" else "无")
	赏赐.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	赏赐.add_theme_stylebox_override("normal", 标题底)
	内容.add_child(赏赐)
	var 日志标 := Label.new()
	日志标.text = "—— 战斗日志 ——"
	...
	确.pressed.connect(func():
		...
		刷新()
		if 胜:
			_刷新历练秘境()  # 重建秘境卡，刷新解锁状态
		# 入门指引推进/收尾（结算遮罩已销毁，此时弹入门指引不会被盖住）
		if 胜 and Game.引导阶段 <= 4:
			# 历练战斗胜利 = 入门指引流程自然终点（殿阁→招收→历练已完成）
...
func _填_殿阁页(内容: Control):
	...
	刷新()
	内容.add_child(_宗门大阵面板())   # S1 批6-A：宗门大阵面板挂殿阁二级页

# === S1 批6-A：宗门大阵面板（挂「殿阁」二级页，纯经营展示 · 零战斗触碰）===
...
func _殿阁点击彩蛋(key: String):
	var egg_id: String = 殿阁彩蛋映射.get(key, "")
	if egg_id == "":
		return
	殿阁点击计数[key] = 殿阁点击计数.get(key, 0) + 1
	var 阈值: int = 殿阁彩蛋阈值.get(key, 3)
	if 殿阁点击计数[key] >= 阈值:
		殿阁点击计数[key] = 0
		Game.触发点击彩蛋(egg_id)
...
func _殿阁总览_填充(列: VBoxContainer, 类别: String):
	for c in 列.get_children():
		c.queue_free()
	for key in 殿阁类别.get(类别, []):
		列.add_child(_殿阁块(key))

func _殿阁总览_汇总栏() -> Control:
	var p := PanelContainer.new()
	p.add_theme_stylebox_override("panel", _暗墨面板())
	var vb := VBoxContainer.new()
	vb.add_theme_constant_override("separation", 2)
	p.add_child(vb)
	var 殿阁数: int = Game.司职列表.size()
	var 在岗: int = _在岗弟子数()
	...
	var l1 := Label.new()
	l1.text = "宗门殿阁 %d 座 ｜ 在岗弟子 %d 人" % [殿阁数, 在岗]
	l1.add_theme_font_size_override("font_size", FONT_AUX)
...
func _在岗弟子数() -> int:
	# 成员按司职分区，互不重叠；此处按姓名去重计（≈ 全体弟子中已入堂者）
	var 集合: Dictionary = {}
	for key in Game.司职列表.keys():
		for m in Game.司职列表[key]["成员"]:
			var d: Disciple = m as Disciple
			集合[d.姓名] = true
	return 集合.size()

func _殿阁卡片底色(类别: String) -> StyleBoxFlat:
	# 复用 造主题 面板形制（宣纸亮底/圆角/边宽），仅边框色按类别取现有色值常量
	var sb := StyleBoxFlat.new()
	sb.bg_color = 宣纸亮
	sb.set_corner_radius_all(8)
	sb.set_border_width_all(1)
	sb.border_color = 殿阁类别色.get(类别, 暗金)
	sb.content_margin_left = 10; sb.content_margin_right = 10
	sb.content_margin_top = 8; sb.content_margin_bottom = 8
	return sb

func _殿阁块(key: String) -> Control:
	var 堂: Dictionary = Game.司职列表[key]
	...
	pc.add_theme_stylebox_override("panel", _殿阁卡片底色(殿阁_类别反查.get(key, "战略")))
	var 内 := VBoxContainer.new()
	...
	负责行.add_child(负责标签)
	if 负责 != null and 殿阁全局维度.has(key):
		var 维: String = 殿阁全局维度[key]
	...
	if 成员.is_empty():
		...
		for m in 成员:
			...
			var 行 := HBoxContainer.new()
			var 名按钮 := Button.new(); 名按钮.text = mm.姓名; 名按钮.pressed.connect(_on_殿阁成员_看详情.bind(mm))
			行.add_child(名按钮)
			var 任按钮 := Button.new()
			if 锁定:
	...
	# S1 批2：殿阁升级入口（展示目标等级/消耗 + 升级按钮，点击展开详情确认）
	var 提示: Dictionary = _殿阁_升级提示(key)
	var 目标等级: int = int(提示["等级"]) + 1
	var 升级文本: String = "升级 → 第 %d 重 · 灵石 -%d" % [目标等级, int(提示["耗"])]
	if bool(提示["可升"]):
		升级文本 += " · 可升级"
	else:
		升级文本 += " · " + str(提示["原因"])
	...
	升按钮.pressed.connect(func(): _打开殿阁详情(key))
	升级行.add_child(升按钮)
	内.add_child(升级行)
	if 锁定:
...
func _on_任负责人(key: String, m: Disciple):
	var 旧主事: Variant = Game.司职列表[key].get("负责人", null)
	if 旧主事 != null and 旧主事 != m and int(Game.司职列表[key].get("政绩", 0)) >= 12:
		_弹出高政绩撤换确认(key, m)
		return
	Game.任命负责人(key, m)
	详情.text = "已任命 %s 为%s主事。" % [m.姓名, Game.司职列表[key]["名称"]]
	_建_二级页("殿阁")

func _on_解除锁定(key: String):
	Game.解除负责人锁定(key)
	详情.text = "已解除%s主事锁定。" % Game.司职列表[key]["名称"]
	_建_二级页("殿阁")

func _on_殿阁成员_看详情(d: Disciple):
	# 复用主界面弟子卡同款入口 _on_选弟子（写入底部详情栏）；关闭本弹窗以露出详情栏
	var 旧遮: Node = get_node_or_null("殿阁总览")
	if 旧遮 != null:
		旧遮.queue_free()
	_on_选弟子(d)

func 殿阁附带效果(key: String) -> String:
	# 说明文字从阶段2 殿阁被动逻辑读取；dormant 标注「待实装」
	match key:
		"lingtian":
...
# ============ S1 批2：殿阁升级 + 任期政绩 UI（§二 / §三）============

# 升级可用性与原因（供总览卡片与详情弹窗共用，避免逻辑重复）
func _殿阁_升级提示(key: String) -> Dictionary:
	var 等级: int = int(Game.司职列表[key].get("等级", 1))
	var 上限: int = Game._殿阁等级上限()
	var 耗: int = Game._升级消耗_灵石(等级)
	var 可升: bool = (等级 < 上限) and (Game.门派等级 >= 2) and (Game.灵石 >= 耗)
	var 原因: String = ""
	if 等级 >= 上限:
		原因 = "已达宗门品级上限（第 %d 重）" % 上限
	elif Game.门派等级 < 2:
		原因 = "需灵阶及以上宗门方可升级"
	elif Game.灵石 < 耗:
...
# 执行升级：调用 Game.升级殿阁，toast 反馈；成功则关弹窗并重建殿阁页
func _执行殿阁升级(key: String) -> void:
...
# 殿阁详情弹窗：等级/上限、任期政绩、当前产出、升级按钮（含消耗与禁用原因）
func _打开殿阁详情(key: String) -> void:
	var 堂: Dictionary = Game.司职列表[key]
	var 等级: int = int(堂.get("等级", 1))
	var 上限: int = Game._殿阁等级上限()
	var 政绩: int = int(堂.get("政绩", 0))
	var 提示: Dictionary = _殿阁_升级提示(key)
	var 弹: Dictionary = _new_detail_popup("%s · 第 %d 重" % [堂["名称"], 等级], 暗金)
	var 内容: VBoxContainer = 弹["内容"] as VBoxContainer
	内容.add_child(_label("殿阁品级 第%d重 / 上限 第%d重（由宗门品级驱动）" % [等级, 上限], FONT_AUX, 次墨))
	var 政绩色: Color = 次墨
	if 政绩 > 0:
		政绩色 = 朱砂
	var 政绩徽: String = ""
	...
	if bool(提示["可升"]):
		内容.add_child(_label("升级至 第 %d 重 需 灵石 -%d（产出 +2%%）" % [等级 + 1, int(提示["耗"])], FONT_BODY, 玉石绿))
	else:
		内容.add_child(_label("升级至 第 %d 重 需 灵石 -%d" % [等级 + 1, int(提示["耗"])], FONT_BODY, 次墨))
	var 升按钮 := Button.new()
	升按钮.text = "升级至 第 %d 重（灵石 -%d）" % [等级 + 1, int(提示["耗"])]
	升按钮.disabled = not bool(提示["可升"])
	升按钮.pressed.connect(func(): _执行殿阁升级(key))
	内容.add_child(升按钮)
...
func _弹出高政绩撤换确认(key: String, m: Disciple) -> void:
	var 旧政绩: int = int(Game.司职列表[key].get("政绩", 0))
	var 弹: Dictionary = _new_detail_popup("撤换主事确认", 朱砂)
	var 内容: VBoxContainer = 弹["内容"] as VBoxContainer
	内容.add_child(_label("%s 现任主事任期内政绩已达 %d，撤换将清零其政绩、开启新任期。" % [Game.司职列表[key]["名称"], 旧政绩], FONT_BODY, 墨黑))
	内容.add_child(_label("确认撤换为 %s？" % m.姓名, FONT_BODY, 墨黑))
	var 确认 := Button.new(); 确认.text = "确认撤换"
	确认.pressed.connect(func():
		Game.任命负责人(key, m)
		详情.text = "已任命 %s 为%s主事（前任政绩清零）。" % [m.姓名, Game.司职列表[key]["名称"]]
		if is_instance_valid(弹["遮"]):
			弹["遮"].queue_free()
		_建_二级页("殿阁")
	)
...
func _皮肤纹理装填(t: Theme):
	if not 启用皮肤纹理:
		return
	var 面板路 := "res://art/_references/ai_art/2026-07-22/游戏UI弹窗面板_玉石半透明质感_宣纸纤维纹理_旧金描边_轻_2026-07-22T15-19-56.png"
	var 按钮路 := "res://art/_references/ai_art/2026-07-22/游戏UI按钮_暗金边框_青玉底色_轻微浮雕_古风器物感_鎏金_2026-07-22T15-19-56.png"
...
# 深墨无边框卡片（登录/选择列表项；比 _暗墨面板 更透、无金边，去除页游堆叠感）
func _暗墨卡片() -> StyleBoxFlat:
	var s := StyleBoxFlat.new()
	s.bg_color = Color(面板底.r, 面板底.g, 面板底.b, 0.55)
	s.set_corner_radius_all(10)
	s.content_margin_left = 12; s.content_margin_right = 12; s.content_margin_top = 10; s.content_margin_bottom = 10
	return s

# 衬线宋体（与开屏页统一审美）：基字体缺失时退回主题默认，不报错
func _取衬线字体() -> Font:
	if ResourceLoader.exists("res://art/fonts/SourceHanSerifCN-Regular.otf"):
		return load("res://art/fonts/SourceHanSerifCN-Regular.otf") as Font
	return null

# 装衬线宋体：可选加字距（FontVariation.spacing_glyph）
func _装衬线字体(控件: Control, 字号: int, 字距: int = 0) -> void:
	var 基: Font = _取衬线字体()
	if 基 != null:
		if 字距 > 0:
	...
	match _type:
		...
		"buff":
			...
			if not bf.is_empty():
				return bf
		"building":
			var bd: Variant = Game.司职列表.get(_id)
			if bd is Dictionary:
				return bd
		"beast":
			for 灵 in Game.灵兽库存:
...
# S1 预埋：从富文本 url 解析并打开详情（战斗日志/奇遇文本/差事描述点击直接复用）
