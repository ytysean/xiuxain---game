...
#
# 作为 pre_f5_check.py 第 11 道 subprocess 闸（紧接七载赏赐零通胀校验之后，复用 run_check 编排）。
...
def main():
    ...
    # (基线键, 提取正则, 说明)
    A_RULES = [
        ("殿阁保底津贴", r"const 殿阁保底津贴:\s*int\s*=\s*(\d+)"),
        ("升级消耗基数", r"ceil\(\s*([\d.]+)\s*\*\s*pow"),
        ("升级消耗指数", r"pow\(\s*([\d.]+)\s*,"),
        ("差事赏赐系数上限", r"clamp\(\s*1\.0\s*\+\s*\(门派等级\s*-\s*1\)\s*\*\s*0\.1,\s*1\.0,\s*([\d.]+)\)"),
        ("随机事件赏赐系数", r'_校准\(\s*"随机事件赏赐系数",\s*([\d.]+)\s*\)'),
        ("招徒基础概率", r'_校准\(\s*"招徒基础概率",\s*([\d.]+)\s*\)'),
        ("招徒概率上限", r'_校准\(\s*"招徒概率上限",\s*([\d.]+)\s*\)'),
        ("气运产出加成", r"设置气运buff\(0\.03,\s*([\d.]+),\s*7\)"),
        ("等级乘区步进", r"var 等级乘区: float = 1\.0 \+ ([\d.]+) \* max"),
        ("殿阁等级乘区步进", r"return 1\.0 \+ ([\d.]+) \* max\(0, 等级 - 1\)"),
    ]
    ...
    # ========================================================================
    # ECON-01 §三 标准局：L=4 / N=48(12殿阁各4人) / BL=2 / 经营加成均+10% /
    ...
    殿阁等级乘区 = 1.0 + 0.02 * max(0, BL - 1)    # 1.02
    保底 = BL * 5                                # 10 (BL>=2)
    单殿阁因子 = (1.0 + 经营加成) * 气运乘 * (1.0 + 产出buff) * 等级乘区 * 殿阁等级乘区
    殿阁产出 = sum(n_per * 经营基数[k] * 单殿阁因子 for k in 经营基数) + 12 * 保底
    复算月产 = 殿阁产出 + N * 2                   # 预估月产出()：殿阁 + 弟子津贴
    ...
    # C3) F3 经营加成 clamp（ECON-01 §4.1）：同 PR 补 clamp(单殿阁≤0.20/全≤0.30)。
    #     按 ECON-01 §4.6 门控：仅当 F2_阀门_接线==1 且 F3_经营加成_封顶==0.30 升级为硬阻断；
    #     本差事仅落地 F2，F3 未做 → WARN 不阻断（保证 pre_f5 25 闸全绿）。
    if f3 == "0.30":
        if not re.search(r"经营加成\s*=\s*clamp\([^,]+,\s*0\.0,\s*0\.20?\)", gs):
            fail("经营加成未封顶至≤0.20/单殿阁（违反§4.1 产出效率池，F3 未落地）")
        if not re.search(r"经营加成_total\s*=\s*clamp\([^,]+,\s*0\.0,\s*0\.30\)", gs):
            fail("全殿阁经营加成未封顶至≤0.30（违反§4.1 产出效率池，F3 未落地）")
    else:
        warn("F3 经营加成 clamp 未落地（F3_经营加成_封顶=%s，本差事仅做 F2；同 PR 补，ECON-01 Layer C 门控）" % f3)
    ...
    # ========================================================================
    # T3 —— 符词条·战力 子帽静态守（PH6-B4，落点在 disciple.gd 侧）
    #   背景：C1 把符词条·战力 从 base（乘性主体之前）迁入 聚合通用增益() 统一池；
    #         B4 裁决 A-各自 ⇒ 给「符箓来源」单独加 8% 子帽（与装备来源各自独立）。
    #   守两件：① 计算战力() 不得再出现 flat 注入 base（防回退）；
    #           ② 符箓池必须经 clamp(符箓词缀池, 0.0, 0.08) 夹帽。
    # ========================================================================
    _disc3_path = os.path.join(ROOT, "disciple.gd")
    if not os.path.exists(_disc3_path):
        fail("disciple.gd 缺失，无法静态守符词条·战力子帽（PH6-B4）")
    _disc3 = open(_disc3_path, "r", encoding="utf-8").read()
    if "base += float(Game.护身符战力加成(self))" in _disc3:
        fail("符词条·战力 回退为 flat 注入 base（违反 PH6-C1：须经 聚合通用增益() 归一化+8%子帽入池）")
    if not re.search(r"clamp\(\s*符箓词缀池\s*,\s*0\.0\s*,\s*0\.08\s*\)", _disc3):
        fail("符词条·战力 缺少单源子帽 clamp(符箓词缀池, 0.0, 0.08)（违反 PH6-B4）")
    print("T3 已通过：符词条·战力 无 flat 注入 base · 符箓池单源子帽 clamp≤0.08 在位")
