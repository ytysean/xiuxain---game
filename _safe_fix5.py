...
for i in range(len(lines)):
    ...
    # 2. _传承事件参数缺引号: _传承事件("xxx) → _传承事件("xxx")
    # 只处理简单的情况，参数中不包含引号、括号、百分号
    matches = re.findall(r'_传承事件\("([^"]*?)\)', line)
    for m in matches:
        if ('"' not in m and '(' not in m and ')' not in m and 
            '%' not in m and '+' not in m and '#' not in m):
            old = f'_传承事件("{m})'
            new = f'_传承事件("{m}")'
            if old in line:
                line = line.replace(old, new)
                fixes.append((i+1, f'_传承事件参数补引号: {m}', original.strip()[:80], line.strip()[:80]))
