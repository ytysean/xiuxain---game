...
func _建_酝造() -> void:
	...
	var _fb2 := _行("酿道品级：%d（修为 %d）· 今日已酝 %d / 5" % [int(Game.酿道等级), int(Game.酿道经验), int(Game.今日酿酒次数)], UITheme.COLOR_TEXT_BODY_GOLD)
	_content.add_child(_fb2)
	_fb2.modulate.a = 0.0
	_fb2.create_tween().tween_property(_fb2, "modulate:a", 1.0, 0.25)
	var 配方表: Array = Game.获取灵酿配方列表()
	var _fb3 := _说明("已解锁 %d 道配方；酝造耗灵草，入窖待其自熟，熟则入库。" % 配方表.size())
...
func _建_在酿() -> void:
	...
	if Game.灵酿列表.is_empty():
		var _fb7 := _行("窖中暂无在酿之酒。", UITheme.COLOR_TEXT_AUX)
	...
	for 酿 in Game.灵酿列表:
		...
		var _fb8 := _行("%s · 尚需 %d 日" % [str(酿.get("名称", "")), max(0, 余)], UITheme.COLOR_TEXT_BODY_GOLD if 余 <= 0 else UITheme.COLOR_TEXT_BODY)
		_content.add_child(_fb8)
		_fb8.modulate.a = 0.0
		_fb8.create_tween().tween_property(_fb8, "modulate:a", 1.0, 0.25)
		var _fb9 := _说明("    %s" % str(酿.get("效果", "")))
...
func _建_宴席() -> void:
	...
	var _fb11 := _说明("以灵酿宴请全宗，弟子欢欣，忠诚同增。")
	...
	for it in Game.宗门库房:
		...
		按钮.create_tween().tween_property(按钮, "modulate:a", 1.0, 0.25)
	if 数 == 0:
		var _fb12 := _行("库中暂无成酿，先去酝造。", UITheme.COLOR_TEXT_AUX)
